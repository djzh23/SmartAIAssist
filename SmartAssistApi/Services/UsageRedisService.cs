...
public class UsageRedisService(IConfiguration config, HttpClient http)
{
    ...
    public virtual Task<string?> GetStripeLastEventIdAsync(string userId) =>
        RedisGetAsync($"stripe_last_event:{userId}");

    public virtual Task<string?> GetStripeLastEventAtAsync(string userId) =>
        RedisGetAsync($"stripe_last_event_at:{userId}");

    public virtual Task<string?> GetStripeLastSessionIdAsync(string userId) =>
        RedisGetAsync($"stripe_last_session:{userId}");
    ...
    public virtual async Task<StripeDebugInfo> GetStripeDebugInfoAsync(string userId)
    {
        ...
        var lastEventId = await GetStripeLastEventIdAsync(userId);
        var lastEventAt = await GetStripeLastEventAtAsync(userId);
        var lastSessionId = await GetStripeLastSessionIdAsync(userId);
